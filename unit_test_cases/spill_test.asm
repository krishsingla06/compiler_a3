...
main:
    ...
    # DEBUG: Spilling register $t4 due to register pressure
    sw $t4, 0($fp)
    # DEBUG: Spilled #t1 from $t4 to memory at 0($fp)
    lw $t4, -32($fp)
    ...
    # DEBUG: Spilling register $t7 due to register pressure
    sw $t7, 0($fp)
    # DEBUG: Spilled #t2 from $t7 to memory at 0($fp)
    lw $t7, -36($fp)
    # DEBUG: Loaded v_i_main_s2 from memory at -36($fp)
    # DEBUG: v_i_main_s2 in $t7
    # DEBUG: Spilling register $t8 due to register pressure
    sw $t8, 0($fp)
    # DEBUG: Spilled #t3 from $t8 to memory at 0($fp)
    lw $t8, -40($fp)
    ...
    # DEBUG: Spilling register $t1 due to register pressure
    sw $t1, 0($fp)
    # DEBUG: Spilled #t4 from $t1 to memory at 0($fp)
    lw $t1, -44($fp)
    # DEBUG: Loaded v_k_main_s2 from memory at -44($fp)
    # DEBUG: v_k_main_s2 in $t1
    # DEBUG: Spilling register $t2 due to register pressure
    sw $t2, 0($fp)
    # DEBUG: Spilled #t5 from $t2 to memory at 0($fp)
    lw $t2, -48($fp)
    ...
    sw $t3, 0($fp)
    # DEBUG: Spilled #t10 from $t3 to memory at 0($fp)
    sw $t4, 0($fp)
    # DEBUG: Spilled #t11 from $t4 to memory at 0($fp)
    sw $t4, -96($fp)
    # DEBUG: Spilled v_result_main_s2 from $t4 to memory at -96($fp)
    sw $t5, 0($fp)
    # DEBUG: Spilled #t6 from $t5 to memory at 0($fp)
    sw $t6, 0($fp)
    # DEBUG: Spilled #t7 from $t6 to memory at 0($fp)
    sw $t7, -36($fp)
    # DEBUG: Spilled v_i_main_s2 from $t7 to memory at -36($fp)
    sw $t8, -40($fp)
    # DEBUG: Spilled v_j_main_s2 from $t8 to memory at -40($fp)
    sw $t9, 0($fp)
    # DEBUG: Spilled #t8 from $t9 to memory at 0($fp)
    ...
    # v_k_main_s2: [$t1, memory:-44($fp)]
    # v_l_main_s2: [$t2, memory:-48($fp)]
    # v_result_main_s2: [$t4, memory:-96($fp)]
    # --- End Storage Descriptor ---
